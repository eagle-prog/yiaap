...
function uploadStart(file) {
	try {
		...
		progress.toggleCancel(true, this);
		/* progress display */
		$("#progress-stats").removeClass("no-display");
...
function uploadComplete(file) {
	...
	var the_form = "#upload-form";
	var the_url = $(the_form).attr("action");

	if($("#total-uploads").val() == '0'){
	    $("#fsUploadStats").load(the_url+"&do=reload-stats");
	}
...
function queueComplete(numFilesUploaded) {
    $("#progress-stats").addClass("no-display");
}
