...
(function($) {
    ...
    $.fn.captcha = function(param) {
        ...
        var submit = $(this).find('button.form-button');
        submit.attr('disabled', 'disabled');

        // Insert captcha text and input before the submit button with the given ID's
        $('<div class="captcha_div"><label id="bot_label">I am not a bot: </label><label id="' + params.idCaptchaText + '"></label><input id="' + params.idCaptchaInput + '" aria-label="Captcha Input" type="text" required></div>').insertBefore(submit);
