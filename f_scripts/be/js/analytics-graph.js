...
//    clientid: '1005570616300-okfur9810bd7mkmldo5uk99q53r0su5q.apps.googleusercontent.com',
...
/**
   * Draw the a chart.js line chart with data from the specified view that
   * overlays session data for the current month over session data for the
   * previous month.
   */
  ...
  function renderMonthOverMonthChart(ids) {
    ...
    Promise.all([thisMonth, lastMonth]).then(function(results) {
      ...
      labels = labels.map(function(label) {
        return moment(label, 'YYYYMMDD').format('DD dd');
      });
...
//	var cdata = [];
      data = [];
      data[0] = ['Country', 'Sessions'];
      var xcolors = ['#4D5360','#949FB1','#D4CCC5','#E2EAE9','#F7464A', '#1d72aa', '#8cbb4e', '#795892', '#0099b2', '#f38533'];
      var tcolors = ['#aadff3', '#058dc7'];

      response.rows.forEach(function(row, i) {
      	  data[i+1] = [row[0], +row[1]];
      });

       var options = {tooltip: {isHtml: true}, colorAxis: { colors: tcolors}};
       var data = google.visualization.arrayToDataTable(data);
       data.addColumn({type: 'string', role: 'tooltip'});

        var chart = new google.visualization.GeoChart(document.getElementById('chart-4-container'));
