@section('site_title', formatTitle([$website->url, __('Countries'), config('settings.title')]))
                                        ...
                                        @if(isset($first->value))
                                            ...
                                            <div class="flex-grow-1 font-weight-bold text-truncate {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                @if(!empty(explode(':', $first->value)[1]))
                                                    <a href="{{ route('stats.cities', ['id' => $website->url, 'search' => explode(':', $first->value)[0].':', 'from' => $range['from'], 'to' => $range['to']]) }}" class="text-body">{{ explode(':', $first->value)[1] }}</a>
                                                @else
                                                    {{ __('Unknown') }}
                                                @endif
                                            </div>
                                        ...
                                        @if(isset($last->value))
                                            ...
                                            <div class="flex-grow-1 font-weight-bold text-truncate {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                @if(!empty(explode(':', $last->value)[1]))
                                                    <a href="{{ route('stats.cities', ['id' => $website->url, 'search' => explode(':', $last->value)[0].':', 'from' => $range['from'], 'to' => $range['to']]) }}" class="text-body">{{ explode(':', $last->value)[1] }}</a>
                                                @else
                                                    {{ __('Unknown') }}
                                                @endif
                                            </div>
                        ...
                        <div class="list-group-item px-0 border-0">
                            <div class="d-flex flex-column">
                                ...
                                <div class="progress chart-progress w-100">
                                    <div class="progress-bar bg-visitor rounded" role="progressbar" style="width: {{ (($country->count / $total->count) * 100) }}%"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
