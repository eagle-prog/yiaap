<div class="d-flex">
    <nav class="navbar navbar-expand-xl navbar-light w-100 p-0 mt-3 bg-base-0 rounded shadow-sm">
        <div class="d-flex align-items-center d-xl-none px-3 py-3 font-weight-medium text-muted">
            @php
                $menu = [
                    ...
                    'stats.landing-pages' => ['Landing pages', 'landing'],
                    'stats.referrers' => ['Referrers', 'link'],
                    'stats.search-engines' => ['Search engines', 'search'],
                    'stats.social-networks' => ['Social networks', 'social'],
                    ...
                    'stats.operating-systems' => ['Operating systems', 'os'],
                    'stats.browsers' => ['Browsers', 'browser'],
                    'stats.screen-resolutions' => ['Screen resolutions', 'screen_resolution'],
        ...
        <button class="navbar-toggler border-0 py-2 collapsed {{ (__('lang_dir') == 'rtl' ? 'mr-auto' : 'ml-auto') }}" type="button" data-toggle="collapse" data-target="#navbarStatsMenu" aria-controls="navbarTogglerDemo02" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon my-1"></span>
        </button>

        <div class="collapse navbar-collapse border-top border-xl-top-0" id="navbarStatsMenu">
            <ul class="navbar-nav flex-wrap justify-content-between w-100">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link d-flex align-items-center font-weight-medium py-3 px-3 {{ in_array(Route::currentRouteName(), ['stats.pages', 'stats.landing-pages']) ? 'active' : '' }}" href="#" id="behaviorDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ...
                    <div class="dropdown-menu border-0 shadow {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu-right' : 'dropdown-menu') }}" aria-labelledby="behaviorDropdown">
                        ...
                        <a class="dropdown-item {{ Route::currentRouteName() == 'stats.landing-pages' ? 'active' : '' }}" href="{{ route('stats.landing-pages', ['id' => $website->url, 'from' => $range['from'], 'to' => $range['to']]) }}">{{ __('Landing pages') }}</a>
                    </div>
                </li>

                <li class="nav-item dropdown">
                    <a class="nav-link d-flex align-items-center font-weight-medium py-3 px-3 {{ in_array(Route::currentRouteName(), ['stats.referrers', 'stats.campaigns', 'stats.search-engines', 'stats.social-networks']) ? 'active' : '' }}" href="#" id="acquisitionsDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ...
                    <div class="dropdown-menu border-0 shadow {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu-right' : 'dropdown-menu') }}" aria-labelledby="acquisitionsDropdown">
                        ...
                        <a class="dropdown-item {{ Route::currentRouteName() == 'stats.search-engines' ? 'active' : '' }}" href="{{ route('stats.search-engines', ['id' => $website->url, 'from' => $range['from'], 'to' => $range['to']]) }}">{{ __('Search engines') }}</a>

                        <a class="dropdown-item {{ Route::currentRouteName() == 'stats.social-networks' ? 'active' : '' }}" href="{{ route('stats.social-networks', ['id' => $website->url, 'from' => $range['from'], 'to' => $range['to']]) }}">{{ __('Social networks') }}</a>
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link d-flex align-items-center font-weight-medium py-3 px-3 {{ in_array(Route::currentRouteName(), ['stats.browsers', 'stats.operating-systems', 'stats.screen-resolutions', 'stats.devices']) ? 'active' : '' }}" href="#" id="geographicDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        @include('icons.platforms', ['class' => 'fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2')])
                        <div>{{ __('Technology') }}</div>
                        @include('icons.expand', ['class' => 'fill-current width-3 height-3 '.(__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-2')])
                    </a>
                    <div class="dropdown-menu border-0 shadow {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu-right' : 'dropdown-menu') }}" aria-labelledby="geographicDropdown">
                        <a class="dropdown-item {{ Route::currentRouteName() == 'stats.operating-systems' ? 'active' : '' }}" href="{{ route('stats.operating-systems', ['id' => $website->url, 'from' => $range['from'], 'to' => $range['to']]) }}">{{ __('Operating systems') }}</a>
                        ...
                        <a class="dropdown-item {{ Route::currentRouteName() == 'stats.screen-resolutions' ? 'active' : '' }}" href="{{ route('stats.screen-resolutions', ['id' => $website->url, 'from' => $range['from'], 'to' => $range['to']]) }}">{{ __('Screen resolutions') }}</a>
