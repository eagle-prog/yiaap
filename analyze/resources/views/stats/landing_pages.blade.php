@section('site_title', formatTitle([$website->url, __('Landing pages'), config('settings.title')]))
            ...
            <div class="col-12 col-md-auto d-none d-xl-flex align-items-center border-bottom border-md-bottom-0 {{ (__('lang_dir') == 'rtl' ? 'border-md-left' : 'border-md-right') }}">
                <div class="px-2 py-4 d-flex">
                    <div class="d-flex position-relative text-primary width-10 height-10 align-items-center justify-content-center flex-shrink-0">
                        ...
                        @include('icons.landing', ['class' => 'fill-current width-5 height-5'])
                    </div>
                </div>
            </div>
                                    ...
                                    <div class="d-flex align-items-center text-truncate">
                                        @if(isset($first->value))
                                            <div class="d-flex flex-grow-1 font-weight-bold text-truncate {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                @if($first->value)
                                                    <div class="text-truncate" dir="ltr">{{ $first->value }}</div> <a href="http://{{ $website->url . $first->value }}" target="_blank" rel="nofollow noreferrer noopener" class="text-secondary d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-2') }}">@include('icons.open_new', ['class' => 'fill-current width-3 height-3'])</a>
                                                @else
                                                    {{ __('Unknown') }}
                                                @endif
                                            </div>
                                ...
                                <div class="col">
                                    <div class="d-flex align-items-center text-truncate">
                                        @if(isset($last->value))
                                            <div class="d-flex flex-grow-1 font-weight-bold text-truncate {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                @if($last->value)
                                                    <div class="text-truncate" dir="ltr">{{ $last->value }}</div> <a href="http://{{ $website->url . $last->value }}" target="_blank" rel="nofollow noreferrer noopener" class="text-secondary d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-2') }}">@include('icons.open_new', ['class' => 'fill-current width-3 height-3'])</a>
                                                @else
                                                    {{ __('Unknown') }}
                                                @endif
                                            </div>
                        ...
                        <div class="list-group-item px-0 border-0">
                            <div class="d-flex flex-column">
                                <div class="d-flex justify-content-between mb-2">
                                    <div class="d-flex text-truncate align-items-center">
                                        <div class="d-flex text-truncate">
                                            <div class="text-truncate" dir="ltr">{{ $landingPage->value }}</div> <a href="http://{{ $website->url . $landingPage->value }}" target="_blank" rel="nofollow noreferrer noopener" class="text-secondary d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-2') }}">@include('icons.open_new', ['class' => 'fill-current width-3 height-3'])</a>
                                        </div>
                                    </div>
                                ...
                                <div class="progress chart-progress w-100">
                                    <div class="progress-bar bg-visitor rounded" role="progressbar" style="width: {{ (($landingPage->count / $total->count) * 100) }}%"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
