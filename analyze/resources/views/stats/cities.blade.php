@section('site_title', formatTitle([$website->url, __('Cities'), config('settings.title')]))
            ...
            <div class="col-12 col-md-auto d-none d-xl-flex align-items-center border-bottom border-md-bottom-0 {{ (__('lang_dir') == 'rtl' ? 'border-md-left' : 'border-md-right') }}">
                <div class="px-2 py-4 d-flex">
                    <div class="d-flex position-relative text-primary width-10 height-10 align-items-center justify-content-center flex-shrink-0">
                        ...
                        @include('icons.city', ['class' => 'fill-current width-5 height-5'])
                    </div>
                </div>
            </div>
                        ...
                        <div class="list-group-item px-0 border-0">
                            <div class="d-flex flex-column">
                                <div class="d-flex justify-content-between mb-2">
                                    <div class="d-flex text-truncate align-items-center">
                                        <div class="d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}"><img src="{{ asset('/images/icons/countries/'. formatFlag($city->value)) }}.svg" class="width-4 height-4"></div>
                                        <div class="text-truncate">
                                            @if(!empty(explode(':', $city->value)[1]))
                                                {{ explode(':', $city->value)[1] }}
                                            @else
                                                {{ __('Unknown') }}
                                            @endif
                                        </div>
                                ...
                                <div class="progress chart-progress w-100">
                                    <div class="progress-bar bg-visitor rounded" role="progressbar" style="width: {{ (($city->count / $total->count) * 100) }}%"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
