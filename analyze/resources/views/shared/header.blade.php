...
@else
    ...
    <nav class="slide-menu shadow bg-base-0 ct navbar navbar-light p-0 d-flex flex-column z-1025" id="slide-menu">
        <div class="sidebar-section flex-grow-1 d-flex flex-column w-100">
            ...
            <div class="py-3 px-4">
                @if($userFeatures['option_pageviews'] >= 0)
                    @if($stats['pageviews'] >= $userFeatures['option_pageviews'])
                ...
                <div class="row no-gutters">
                    <div class="col">
                        <div class="small text-muted">
                            {{ __(':number of :total pageviews used.', ['number' => shortenNumber($stats['pageviews']), 'total' => ($userFeatures['option_pageviews'] < 0 ? '∞' : shortenNumber($userFeatures['option_pageviews']))]) }}
                        </div>
                    </div>
                </div>

                <div class="progress w-100 my-2 chart-progress">
                    <div class="progress-bar bg-pageview rounded" role="progressbar" style="width: {{ ($userFeatures['option_pageviews'] == 0 ? 100 : (($stats['pageviews'] / $userFeatures['option_pageviews']) * 100)) }}%"></div>
                </div>
            </div>
