...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        ...
        @if(isset($admin) && !$user->email_verified_at)
            <div class="alert alert-secondary" role="alert">
                {{ __(':name is pending email confirmation.', ['name' => $user->name]) }}
            </div>
        @endif
        ...
        <form action="{{ isset($admin) ? route('admin.users.edit', $user->id) : route('settings.account.update') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="email_account_limit" value="1" class="custom-control-input {{ $errors->has('email_account_limit') ? ' is-invalid' : '' }}" id="customCheckbox2" @if($user->email_account_limit && old('email_account_limit') == null || old('email_account_limit')) checked @endif>
            ...
            @if(isset($admin))
                <hr>
                <div class="form-group">
                    <label for="i_email_verified_at">{{ __('Verified email') }}</label>
                ...
                <div class="col-auto">
                    @if(isset($admin))
                        ...
                        <div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow">
                            @if($user->trashed())
                                <a class="dropdown-item text-success d-flex align-items-center" href="#" data-toggle="modal" data-target="#restoreModal">@include('icons.restore', ['class' => 'fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Restore') }}</a>
                                <div class="dropdown-divider"></div>
                            @else
                                <a class="dropdown-item text-danger d-flex align-items-center" href="#" data-toggle="modal" data-target="#disableModal">@include('icons.block', ['class' => 'fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Disable') }}</a>
                            @endif
                            <a class="dropdown-item text-danger d-flex align-items-center" href="#" data-toggle="modal" data-target="#deleteModal">@include('icons.delete', ['class' => 'fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Delete') }}</a>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="row">
        @php
            $menu = [
                ...
                ['icon' => 'icons.website', 'route' => 'admin.websites', 'title' => __('Websites'), 'stats' => 'websites']
            ];
        @endphp
                ...
                <a href="{{ route($link['route'], ['user_id' => $user->id]) }}" class="text-decoration-none text-secondary">
                    <div class="card border-0 h-100 shadow-sm">
                        <div class="card-body d-flex align-items-center">
                            ...
                            @include((__('lang_dir') == 'rtl' ? 'icons.chevron_left' : 'icons.chevron_right'), ['class' => 'width-3 height-3 fill-current mx-2'])
