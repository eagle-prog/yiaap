...
session_start();
	...
	$settings = get_settings('platform_version');
	...
	if(isset($_GET['page']) && $_GET['page'] == 'settings') {
		...
		if(isset($_GET['smtp']) && $_GET['smtp'] == '1') {

			if(isset($_POST['submit'])) {

				$new_settings = array();
				$new_settings['smtp_host'] = mysqli_real_escape_string($db,$_POST['smtp_host']);
				$new_settings['smtp_user'] = mysqli_real_escape_string($db,$_POST['smtp_user']);
				$new_settings['smtp_pass'] = mysqli_real_escape_string($db,$_POST['smtp_pass']);
				$new_settings['smtp_port'] = mysqli_real_escape_string($db,$_POST['smtp_port']);
				$new_settings['smtp_ssl'] = mysqli_real_escape_string($db,$_POST['smtp_ssl']);
		...
		if(isset($_GET['layout']) && $_GET['layout'] == '1') {
			...
			if(isset($_POST['submit'])) {
				...
				$new_settings['header_type'] = mysqli_real_escape_string($db,$_POST['header_type']);
				$new_settings['header_bgcolor'] = mysqli_real_escape_string($db,$_POST['header_bgcolor']);
				$new_settings['header_logocolor'] = mysqli_real_escape_string($db,$_POST['header_logocolor']);
				$new_settings['header_menucolor'] = mysqli_real_escape_string($db,$_POST['header_menucolor']);
				$new_settings['header_uploadbgcolor'] = mysqli_real_escape_string($db,$_POST['header_uploadbgcolor']);
				$new_settings['header_uploadtxcolor'] = mysqli_real_escape_string($db,$_POST['header_uploadtxcolor']);
				$new_settings['header_homebg'] = mysqli_real_escape_string($db,$_POST['header_homebg']);
				$new_settings['header_homeht'] = mysqli_real_escape_string($db,$_POST['header_homeht']);
				$new_settings['header_homeopacity'] = mysqli_real_escape_string($db,$_POST['header_homeopacity']);
				$new_settings['header_fixed'] = mysqli_real_escape_string($db,$_POST['header_fixed']);
		...
		if(isset($_GET['contests']) && $_GET['contests'] == '1') { 

			if(isset($_POST['submit'])) {

				$new_settings = array();
				$new_settings['user_contest_price'] = mysqli_real_escape_string($db,$_POST['user_contest_price']);
				$new_settings['allow_user_contests'] = mysqli_real_escape_string($db,$_POST['allow_user_contests']);
				$new_settings['contest_entry_fee'] = mysqli_real_escape_string($db,$_POST['contest_entry_fee']);
		...
		if(!isset($_GET['customization']) && !isset($_GET['smtp']) && !isset($_GET['layout']) && !isset($_GET['contests']) && !isset($_GET['watermark']) && !isset($_GET['credentials'])) {
			...
			if(isset($_POST['submit'])) {
				...
				$new_settings['confirm_mail'] = mysqli_real_escape_string($db,$_POST['confirm_mail']);
				$new_settings['content_ffmpeg'] = mysqli_real_escape_string($db,$_POST['content_ffmpeg']);
				$new_settings['redirect_ssl'] = mysqli_real_escape_string($db,$_POST['redirect_ssl']);
				$new_settings['uploader'] = mysqli_real_escape_string($db,$_POST['uploader']);
				$new_settings['youtube'] = mysqli_real_escape_string($db,$_POST['youtube']);
				$new_settings['rtl'] = mysqli_real_escape_string($db,$_POST['rtl']);
				...
				foreach($new_settings as $key=>$val) {
					...
					$check_s = mysqli_query($db,"SELECT * FROM `settings` WHERE `set_key` = '".$key."' LIMIT 1") or die(mysqli_error($db));
					if(mysqli_num_rows($check_s)) {
						mysqli_query($db,"UPDATE `settings` SET `set_value` = '".$val."' WHERE `set_key` = '".$key."' LIMIT 1") or die(mysqli_error($db));
					} else {
						mysqli_query($db,"INSERT INTO `settings` (`set_key`,`set_value`) VALUES ('".$key."','".$val."')") or die(mysqli_error($db));
					}
	...
	if(isset($_GET['extrap']) && $_GET['extrap'] == 'settings') {

		if(isset($_POST['submit'])) {
				
			$new_settings = array();
	
			$new_settings['razorpay_api'] = mysqli_real_escape_string($db,$_POST['razorpay_api']);
			$new_settings['razorpay_secret'] = mysqli_real_escape_string($db,$_POST['razorpay_secret']);
			$new_settings['payment_gateway'] = mysqli_real_escape_string($db,$_POST['payment_gateway']);
			$new_settings['paypal_email'] = mysqli_real_escape_string($db,$_POST['paypal_email']);
			$new_settings['paypal_currency'] = mysqli_real_escape_string($db,$_POST['paypal_currency']);
	...
	if(!isset($site_settings) && isset($_GET['extrap']) && $_GET['extrap'] == 'settings') {
		$site_settings = get_settings();
	}
...
<body>
	...
	<div class="mobile fixed_head">
		...
		<div class="mobile_slogan">v<?=(isset($settings['platform_version']) ? $settings['platform_version'] : '1.4.2');?></div>
	</div>
	...
	<div class="ov_visible">
		<div class="admin_tab_1 menu">
			...
			<div class="alt_desktop">
				...
				<div class="desktop_slogan">version <?=(isset($settings['platform_version']) ? $settings['platform_version'] : '1.4.2');?></div>
			</div>
			...
			<div class="padding_top20">
				...
				<a href="index.php?page=contests">
					<div class="menu_item <?=(isset($_GET['page']) && ($_GET['page'] == 'contest' || $_GET['page'] == 'contests') ? 'menu_item_selected' : '');?>">
						<div class="menu_item_icon"><i class="fas fa-trophy"></i></div>
						<div class="menu_item_text">Contests</div>
					</div>
				</a>
				<a href="index.php?page=payments">
					<div class="menu_item <?=(isset($_GET['page']) && $_GET['page'] == 'payments' ? 'menu_item_selected' : '');?>">
						<div class="menu_item_icon"><i class="fab fa-paypal"></i></div>
						<div class="menu_item_text">Payments</div>
					</div>
				</a>
	...
	function load_payments() {
		...
		var page_nr = $('.payments_results').data('page');
		$('.payments_results').stop().data('page',parseInt(page_nr)+1);

		$.post('request.php', { reason: 'payments', page_nr: page_nr }, function(get) {

			$('.cloading').stop().hide();
			is_loading = 0;

			if(get['payments'].length) {

				var i=0;
				for(i=0;i<=get['payments'].length-1;i++) {

					var result = ''+
					'<div class="users_dash payment payment_'+get['payments'][i].id+'">'+
						'<div class="users_dash_col center payment_dash_col_1">'+get['payments'][i].id+'</div>'+
						'<div class="users_dash_col payment_dash_col_2" style="padding:10px;">'+
							'<span class="bold col_2_dh">'+get['payments'][i].name+'</span><br>'+
							'<span>'+get['payments'][i].email+'</span>'+
						'</div>'+
						'<div class="users_dash_col payment_dash_col_3">'+get['payments'][i].date+'</div>'+
						'<div class="users_dash_col payment_dash_col_4">'+get['payments'][i].product+'</div>'+
						'<div class="users_dash_col payment_dash_col_5">'+get['payments'][i].price+'</div>'+
						'<div class="users_dash_col payment_dash_col_6" '+(get['payments'][i].status == 'Paid' ? 'style="color:#00cc00;"':'')+'>'+get['payments'][i].status+'</div>'+
					'</div>';
	
					$('.payments_results').append(result);

				}

			} else {
				$('.payments_results').stop().data('stop', 1); 
				if(!$('.payment')[0]) {
	...
	$(document).on('click', '.rallr_si', function() {

		$('.pop_ncat').stop().show();
		$('.pop_ncat').stop().data('id','0');

		$('.pop_ncat_head').stop().text('New item');

		$('#item_name').stop().val('');
		$('#item_price, #item_votes, #item_views').stop().val('0');
	...
	$(document).on('click', '.edit_fitem', function() {
	
		$('.pop_ncat_head').stop().text('Edit item');
		...
		$.post('request.php', { reason: 'get_fitem', id: id }, function(get) {
			$('#item_name').stop().val(get.item_name);
			$('#item_votes').stop().val(get.votes);
			$('#item_views').stop().val(get.views);
			$('#item_price').stop().val(get.price);
			$('.pop_ncat').stop().show();
		},'json');
	...
	$(document).on('click', '.more_prizes', function() {

		$('.prizes').append('<div style="margin-bottom:15px;"><input type="text" maxlength="100" name="prize_name[]" placeholder="Prize name" /><br><input type="text" maxlength="1000" name="prize_description[]" placeholder="Prize description (optional)" /></div>');

	});

	$(document).on('click', '.more_rules', function() {

		$('.rules').append('<div style="margin-bottom:15px;"><input type="text" maxlength="250" name="rule_name[]" placeholder="Rule" /></div>');
	...
	$(document).on('click', '.pop_ncat_op_submit_si', function() {

		var item_name = $('#item_name').val();
		var item_votes = $('#item_votes').val();
		var item_views = $('#item_views').val();
		var item_price = $('#item_price').val();

		var id = $('.pop_ncat').data('id');
		if(id == '0') { id = ''; }

		$('.pop_ncat_op_submit_si').stop().html('<i class="fas fa-spinner fa-spin"></i>');
			
		$.post('request.php', { reason: 'update_fitem', fitem_id: id, item_name: item_name, item_votes: item_votes, item_views: item_views, item_price: item_price }, function(get) {
			window.location.reload();
		});
	...
	$(window).scroll(function() {
    		if($(window).scrollTop() > (parseInt($(document).height() - 450) - $(window).height())) {
				...
				<?php if(isset($_GET['page']) && $_GET['page'] == 'payments' && !isset($_GET['extrap'])) { ?>
				if($('.payments_results').data('stop') == '0') {
					load_payments();
				}
				<?php } ?>
			}
			...
			if(get['ratings'].length) {
				...
				for(i=0;i<=get['ratings'].length-1;i++) {
					...
					'<div class="users_dash ratings_dash rat rating_'+get['ratings'][i].id+'">'+
						...
						'<div class="users_dash_col ratings_dash_col_3">'+get['ratings'][i].rate+'</div>'+
						'<div class="users_dash_col ratings_dash_col_4">'+get['ratings'][i].ip+'</div>'+
						'<div class="users_dash_col ratings_dash_col_5">'+get['ratings'][i].date+'</div>'+
						'<div class="users_dash_col center ratings_dash_col_6 remove_rating red2" data-id="'+get['ratings'][i].id+'">&#10005;&nbsp;</div>'+
					'</div>';
	...
	$(document).on('click', '.remove_fitem', function() {

		var id = $(this).data('id');
		$.post('request.php', { reason: 'remove_fitem', id: id });
		$('.fitem_'+id).remove();
	...
	$(document).on('change', '#payment_gateway', function(vals) {

		var new_val = $('#payment_gateway').val();

		if(new_val == 'paypal') {
			$('.p_only_paypal').stop().show();
			$('.p_only_razorpay').stop().hide();
		}

		if(new_val == 'razorpay') {
			$('.p_only_paypal').stop().hide();
			$('.p_only_razorpay').stop().show();
		}
