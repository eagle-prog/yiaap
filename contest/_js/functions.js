(function($){
	...
	$(document).on('click', '.more_prizes', function() {
		...
		$('.prizes').append('<div style="margin-bottom:15px;"><input type="text" maxlength="100" name="prize_name[]" placeholder="'+lang_prize_name+'" /><br><input type="text" maxlength="1000" name="prize_description[]" placeholder="'+lang_prize_desc+'" /></div>');

	});

	$(document).on('click', '.more_rules', function() {

		$('.rules').append('<div style="margin-bottom:15px;"><input type="text" maxlength="250" name="rule_name[]" placeholder="'+lang_rule+'" /></div>');

	});

	$(document).on('click', '.click_create_contest', function() {

		window.location = site_url+'index.php?create_contest=1';

	});

	$(document).on('click', '.click_my_contests', function() {

		window.location = site_url+'index.php?my_contests=1';

	});

	$(document).on('click', '.buy_contest_click', function() {

		var id = $(this).data('id');

		$.post('_core/request.php', { reason: 'paynow_contest', item_id: id }, function(get) {
			...
			if(get.error == '0') {
				...
				if(get.ptype == 'paypal') {
					...
					var pay_form = ''+
					'<form action="'+get['paypal_url']+'" id="paynow_form" method="post">'+
						'<input type="hidden" name="business" value="'+get.paypal_id+'">'+
						'<input type="hidden" name="cmd" value="_xclick">'+
						'<input type="hidden" name="item_name" value="'+get.item_name+'">'+
						'<input type="hidden" name="item_number" value="'+get.item_id+'">'+
						'<input type="hidden" name="amount" value="'+new_contest_price+'">'+
						'<input type="hidden" name="currency_code" value="'+get.currency+'">'+
						'<input type="hidden" name="return" value="'+get.return_url+'?unid='+get.unid+'&contest_id='+id+'">'+
						'<input type="hidden" name="cancel_return" value="'+get.cancel_url+'?unid='+get.unid+'&contest_id='+id+'">'+
 					...
 					loadExternalScript('https://checkout.razorpay.com/v1/checkout.js').then(function() { 
						...
						var options = {
    							...
    							"handler": function (response){
        							window.location = get.return_url+'?unid='+get.unid+'&contest_id='+id;
    							},
						};
	...
	$(document).on('click', '.paynow_joincontest', function() {

		var id = $(this).data('id');

		$.post('_core/request.php', { reason: 'paynow_joincontest', item_id: id }, function(get) {
			...
			if(get.error == '0') {
				...
				if(get.ptype == 'paypal') {
					...
					'<form action="'+get['paypal_url']+'" id="paynow_form" method="post">'+
						...
						'<input type="hidden" name="amount" value="'+new_contest_price+'">'+
						'<input type="hidden" name="currency_code" value="'+get.currency+'">'+
						'<input type="hidden" name="return" value="'+get.return_url+'?unid='+get.unid+'&ptype=2&contest_id='+id+'">'+
						'<input type="hidden" name="cancel_return" value="'+get.cancel_url+'?unid='+get.unid+'&ptype=2&contest_id='+id+'">'+
						'<button type="hidden">&nbsp;</button>'+
					'</form>';
				...
				if(get.ptype == 'razorpay') {
 					...
 					loadExternalScript('https://checkout.razorpay.com/v1/checkout.js').then(function() { 
						...
						var options = {
    							...
    							"handler": function (response){
        							window.location = get.return_url+'?unid='+get.unid+'&ptype=2&contest_id='+id;
    							},
						};
	...
	$(document).on('click', '.buy_votes_close', function() {

		$('.buy_votes_pop').stop().hide();

	});

	$(document).on('click', '.click_ctab', function() {
		
		$('.click_ctab_h').stop().removeClass('click_ctab_h');
		var id = $(this).data('id');
		$('.contest_page_tab').stop().hide();

		$('.contest_page_tab[data-id="'+id+'"]').stop().show();
		$(this).stop().addClass('click_ctab_h');

	});

	$(document).on('click', '.buy_votes_click', function() {
		
		$('.buy_votes_pop').stop().show();
	
		var hi = $('.buying_votes_box').height();
		$('.buy_votes_pop_inner').stop().css('height', (hi+79)+'px');

	});

	$(document).on('click', '.buy_item_click', function() {

		var id = $(this).data('id');

		$.post('_core/request.php', { reason: 'paynow', item_id: id }, function(get) {
			...
			if(get.error == '0') {
				...
				if(get.ptype == 'paypal') {
					...
					'<form action="'+get['paypal_url']+'" id="paynow_form" method="post">'+
						...
						'<input type="hidden" name="item_name" value="'+get.item_name+'">'+
						'<input type="hidden" name="item_number" value="'+get.item_id+'">'+
						'<input type="hidden" name="amount" value="'+get.price+'">'+
						'<input type="hidden" name="currency_code" value="'+get.currency+'">'+
						'<input type="hidden" name="return" value="'+get.return_url+'?unid='+get.unid+'&photo_id='+photo_id+'">'+
						'<input type="hidden" name="cancel_return" value="'+get.cancel_url+'?unid='+get.unid+'&photo_id='+photo_id+'">'+
						'<button type="hidden">&nbsp;</button>'+
					'</form>';
					...
					$('.buy_votes_pop').stop().hide();
 					...
 					loadExternalScript('https://checkout.razorpay.com/v1/checkout.js').then(function() { 
						...
						var options = {
    							...
    							"handler": function (response){
        							window.location = get.return_url+'?unid='+get.unid+'&photo_id='+photo_id;
    							},
						};
	...
	$(document).on('click', '.click_join_contest', function() {
		...
		$.post('_core/request.php', { reason: 'join_contest', contest_id: contest_id, photos: JSON.stringify(myphotos) }, function(get) {
		...
		$.post('_core/request.php', { reason: 'load_my_photos' }, function(get) {
			...
			if(get['my_photos'].length) {
				...
				for(i=0;i<=get['my_photos'].length-1;i++) {
					...
					if(get['my_photos'][i].type == '0') {
						var thumb_picture = site_url+'_uploads/_photos/'+get['my_photos'][i].photo+'_400.jpg';
					}

					if(get['my_photos'][i].type == '1') {
						if(get['my_photos'][i].cover.length > 5) {
							var thumb_picture = site_url+'_uploads/_content_cover/'+get['my_photos'][i].cover+'_400.jpg';
						} else {
							var thumb_picture = site_url+'_img/no_thumb_music.jpg';
						}
					}

					if(get['my_photos'][i].type == '2' || get['my_photos'][i].type == '4') {
						if(get['my_photos'][i].cover.length > 5) {
							var thumb_picture = site_url+'_uploads/_content_cover/'+get['my_photos'][i].cover+'_400.jpg';
						} else {
					...
					'<div class="clmp '+clmp_class+'" data-id="'+get['my_photos'][i].id+'">'+
						clmp_filed+
						'<img src="'+thumb_picture+'" style="width:100%;" />'+
					'</div>';
	...
	if((this_page == 'contests' || this_page == 'my_contests') && $('.new_timer')[0]) {
	...
	function load_rankings(page) {
		...
		get_rankings(order,category,contest);

	}

	function get_rankings(order,category,contest) {
		...
		$.post('_core/request.php', { reason: 'rankings', contest: contest, page_nr: page_nr, order: order, category: category }, function(get) {
			...
			if(get.error == 1) {
				...
				for(i=0;i<=get['list'].length-1;i++) {
					...
					if(get['list'][i].type == '2' || get['list'][i].type == '4') {
						if(get['list'][i].cover.length > 5) {
							var thumb_picture = site_url+'_uploads/_content_cover/'+get['list'][i].cover+'_400.jpg';
						} else {
							var thumb_picture = site_url+'_img/no_thumb_video.jpg';
	...
	function get_photos(page,id2,category=-1) {
		...
		$.post('_core/request.php', { reason: 'photos', page_nr: page_nr, id2: id2, type: page, category: category }, function(ret) {
			...
			for(i=0;i<=ret['files'].length-1;i++) {
				...
				if(ret['files'][i].type == '2' || ret['files'][i].type == '4') {
					if(ret['files'][i].cover.length > 5) {
						var thumb_picture = site_url+'_uploads/_content_cover/'+ret['files'][i].cover+'_400.jpg';
					} else {
						var thumb_picture = site_url+'_img/no_thumb_video.jpg';
	...
	$(document).on('click', '.temp_change_photo_cover', function() {
	...
	$('#upload_photos').ajaxForm({
		...
		complete: function(xhr) {
			...
			if(ret['ok'] == '1' && review_uploader == '1') {

				window.location = site_url+'index.php?uploader=1';
	...
	$(document).on('click', '.click_register', function() {
		...
		$.post('_core/request.php', { role: role, email: email, name: name, password: password, repeat_password: repeat_password, category: category, reason: 'register' }, function(get) {

			if(get.error == '0') {
				if(email_confirm == '1') {
					$('.open_pop[data-id="login"]').click();
					info_er('login',1,get.error_text,600000);
				} else {
					$('#login_pop .pop_inner').stop().css('opacity','0.8');
	...
	$(document).on('click', 'div[data-id="login"]', function() {
		window.location.href = '/signin?redirect=1';
	})

	$(document).on('click', 'div[data-id="register"]', function() {
		window.location.href = '/register?redirect=1';
	})
...
// 		resize_pop(data);
	...
	$(document).on('click', '.click_add_withy', function() {

		if($('.menu_withy').is(':visible')) {
			$('.menu_withy').stop().hide();
		} else {
			$('.menu_withy').stop().show();
		}

	});

	$(document).on('click', '.click_add_youtube', function() {

		if(logged_id != '0') {
			if(mobile == '1') {
				$('.close_menu').click();
			}
			$('#youtube_pop').stop().show();
			$('#youtube_url').stop().val('');
		} else {
			$('.open_pop[data-id="login"]').click();
		}
	...
	$(document).on('click', '.click_submit_youtube', function() {

		var youtube_url = $('#youtube_url').val();
		$.post('_core/request.php', { reason: 'submit_youtube', youtube_url: youtube_url }, function(get) {

			if(get.error == '1') {
				alert(get.error_text);
			} else {

				if(this_is_my_profile == '0' && photo_approval == '1') {
					window.location = site_url+logged_user;
				} else {
					window.location = site_url+'photo-'+get.cid;
				}
	...
	$(document).on('click', '.open_search_m', function() {

		$('.menu_mobile_buttons').stop().hide();
		$('.menu_mobile_search').stop().show();

	});

	$(document).on('click', '.close_menu_search', function() {

		$('.menu_mobile_buttons').stop().show();
		$('.menu_mobile_search').stop().hide();
