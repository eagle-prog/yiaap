<?php
	...
	$path = (strstr($_SERVER['PHP_SELF'],'_core/') || strstr($_SERVER['PHP_SELF'],'_payments') || strstr($_SERVER['PHP_SELF'],'_admin/')? '../': '');
	require_once($path.'_language/english.php');

	function paid_contest_entry($contest_id) {

		global $settings, $db;

		if(isset($settings['contest_entry_fee']) && $settings['contest_entry_fee']) {
			$sql_entry_fee = mysqli_query($db,"SELECT count(*) as 'total' FROM `contest_entry` WHERE `contest_id` = '".$contest_id."' AND `user_id` = '".$_SESSION['_logged_id']."' AND `paid` = '1' LIMIT 1");
			$fetch_entry_fee = mysqli_fetch_array($sql_entry_fee);
			if($fetch_entry_fee['total']) {
				return 1;
			} else {
				return 0;
			}
	...
	function get_exif_photo($imagePath) {
		...
		if ((isset($imagePath)) and (file_exists($imagePath)) && function_exists('read_exif_data')) {

			try {
				$exif_ifd0 = @read_exif_data($imagePath); 
      			} catch (Exception $exp) {
				$exif_ifd0 = false;
			}    
	...
	function get_contest($id=0) {
		...
		if(mysqli_num_rows($check_contest)) {
			...
			return array(
				...
				'creator'=>$fetch_contest['creator'],
				'paid'=>$fetch_contest['paid'],
			);
		} else {
			return array(
				'id'=>0,
	...
	function is_home() {
		...
		if(!isset($_GET['profile']) && !isset($_GET['uploader']) && !isset($_GET['my_contests']) && !isset($_GET['create_contest']) && !isset($_GET['purchase']) && !isset($_GET['contact']) && !isset($_GET['forgot']) && !isset($_GET['contests']) && !isset($_GET['photo']) && !isset($_GET['contest']) && !isset($_GET['extra_page']) && !isset($_GET['settings']) && !isset($_GET['ranking'])) {
			return true;
		}
	...
	function get_youtube_id($url) {
		...
		if(isset($match[1]) && strlen($match[1]) > 8 && strlen($match[1]) < 13) { 
			return $match[1];
		} else {
			return false;
		}
