	...
	<div class="buy_votes_pop">
		<div class="buy_votes_pop_inner">

			<div class="buy_votes_close">✕</div>
			<div class="buy_votes_title"><?=_LANG_BUY_VOTES;?></div>

			<div class="buying_votes_box">

				<?php
				$sql_items = mysqli_query($db,"SELECT * FROM `payments_items` ORDER BY `price` ASC");
				$total_items = mysqli_num_rows($sql_items);
				while($fetch_items = mysqli_fetch_array($sql_items)) {
				
					echo '
					<div class="buy_item_click" data-id="'.$fetch_items['id'].'">
						<span class="itemlist_name">'.$fetch_items['item_name'].'</span><br>
						<span class="itemlist_price">'.$fetch_items['price'].' '.$settings['paypal_currency'].'</span>
					</div>';			
	...
	<div class="main_celling">
		...
		<div class="celling">
			...
			<div class="payment_info_cancel"><?=_LANG_PAYMENT_CANCELED;?></div>
			<?php } ?>

			<?php if(isset($_GET['payment']) && $_GET['payment'] == 'failed') { ?>
			<div class="payment_info_failed"><?=_LANG_PAYMENT_FAILED;?></div>
			<?php } ?>

			<?php if(isset($_GET['payment']) && $_GET['payment'] == 'success') { ?>
			<div class="payment_info_success"><?=_LANG_PAYMENT_SUCCESS;?></div>
			<?php } ?>
			...
			<div class="overflow">
				...
				<div class="photo_page_left overflow">
					...
					<div class="photo_tab_1 <?=(isset($settings['photo_sidebar']) && $settings['photo_sidebar'] == '1' ? 'right':'');?> photo_order_1 overflow photo_tab_type_<?=$fetch_photo['type'];?> <?=($fetch_photo['type'] == '4' ? 'photo_tab_yt':'');?>">
						...
						<?php } elseif($fetch_photo['type'] == '4') { ?>
						<iframe width="100%" height="405" src="https://www.youtube.com/embed/<?=$fetch_photo['photo'];?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
						<?php } ?>
					</div>
				...
				<div class="photo_tab_2 <?=(isset($settings['photo_sidebar']) && $settings['photo_sidebar'] == '1' ? 'left':'');?> relative photo_order_3 overflow">
					...
					<div class="photo_order_2">
						...
						<?php if($fetch_photo['contest'] != '0' || ($multi_contest == '1' && isset($settings['contest_entry_fee']) && $settings['contest_entry_fee'] == '0')) { ?>
						...
						<div class="photo_rating_action">
							...
							<div class="profile_ratings photo_rating_action_box">
								...
								if(isset($settings['content_ratemode']) && $settings['content_ratemode'] == '1') {
									$fetch_photo['rating'] = str_replace('.00','',$fetch_photo['rating']);	
								}
								
								 if($site_contest['active'] == '0' || ($site_contest['active'] == '1' && strtotime($site_contest['end']) > time())) { ?>
						...
						<?php } else { ?>
							...
							<?php if(isset($site_contest['id']) && $site_contest['id'] && $fetch_photo['contest'] == '0') { ?>
							<div class="photo_warning"><?=_LANG_POST_NO_CONTEST;?></div>
							<?php } ?>

							<?php if(!isset($site_contest['id']) || (isset($site_contest['id']) && $site_contest['id'] == '0')) { ?>
							<div class="photo_warning"><?=_LANG_POST_NO_ACTIVE_CONTEST;?></div>
							<?php } ?>
					...
					<?php if($total_items && isset($settings['paypal_email']) && isset($_SESSION['_logged_id'])) { ?>
					<div class="buy_votes_click"><?=_LANG_BUY_VOTES;?></div>
					<?php } ?>
