 ...
 */
function getFileInfo($for){
    include 'f_core/config.database.php';

    $link = mysqli_connect($cfg_dbhost, $cfg_dbuser, $cfg_dbpass, $cfg_dbname);
    if (!$link) {
	echo 'Could not connect: ' . mysqli_connect_errno() . PHP_EOL;
	exit;
    } else {
	$sql 	 = "SELECT `cfg_name`, `cfg_data` FROM `db_settings` WHERE `cfg_name` IN ('server_path_ffmpeg', 'server_path_qt', 'server_path_yamdi', 'server_path_php', 'server_path_lame', 'server_path_unoconv', 'server_path_pdf2swf', 'server_path_convert');";
    ...
    switch($for){
	case "ffmpeg":
	    $_cmd = $cfg['server_path_ffmpeg'].' -version';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
return	    $_ver = implode("<br />", $enc_output);
	break;
	case "qt":
	    $_cmd = $cfg['server_path_ffmpeg'].' -version';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
return	    $_ver = '<span class="black">qt-faststart</span> '.$enc_output[0];
	break;
	case "php":
	    $_cmd = $cfg['server_path_php'].' -v';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
return	    $_ver = implode("<br />", $enc_output);
	break;
	case "apache":
		...
		$_ver = '<span class="black">'.apache_get_version().'</span>';
	    } else {
		$_cmd = '/usr/sbin/httpd -v|grep version';
		exec($_cmd.' 2>&1', $enc_output);
		$enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
		$_ver = implode("<br />", $enc_output);
	    }
	    return $_ver;
	break;
	case "pdf2swf":
	    $_cmd = $cfg['server_path_pdf2swf'].' -V';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
return	    $_ver = implode("<br />", $enc_output);
	break;
...
return	    $_ver = implode("<br />", $enc_output);
	break;
	case "convert":
	    $_cmd = $cfg['server_path_convert'].' --version';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
return	    $_ver = (file_exists($cfg['server_path_convert']) ? '<span class="black">Convert</span> ' : NULL).implode("<br />", $enc_output);
	break;
	case "unoconv":
	    $_cmd = $cfg['server_path_unoconv'].' --version';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output = array_diff($enc_output, array("creation of executable memory area failed: Permission denied"));
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
return	    $_ver = implode("<br />", $enc_output);
	break;
	case "lame":
	    $_cmd = $cfg['server_path_lame'].' --help|grep vers|cut -c-25';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
return	    $_ver = file_exists($cfg['server_path_lame']) ? implode("<br />", $enc_output) : 'sh: '.$cfg['server_path_lame'].': No such file or directory';
	break;
	case "yamdi":
	    $_cmd = $cfg['server_path_yamdi'].' -h';
	    exec($_cmd.' 2>&1', $enc_output);
	    $enc_output[0] = '<span class="black">'.$enc_output[0].'</span>';
	    if($enc_output[1] != ''){
		$_ver = '<span class="black">'.$enc_output[1].'</span><br />';
		$_ver.= $enc_output[2];
	    } else {
		$_ver = implode("<br />", $enc_output);
	    }
...
<div class="time">
	<h1>'.date('H:i:s').'</h1>
	<div class="subtitle">'.date('Y-m-d, e').'</div>
</div>
-->
<br clear="all" />
<div class="col4">
	<div class="vs-column half">
		<div class="infoTable">
			...
			<span class="bold">'.$lang['core'].'</span>
			<i class="fa fa-chevron-down responsive-accordion-plus fa-fw iconBe-plus" style="display: none;"></i>
			<i class="fa fa-chevron-up responsive-accordion-minus fa-fw iconBe-minus" style="display: block;"></i>
			</div>
	...
	if (!empty($settings['show']['ram'])) {
		...
		<div class="infoTable">
			...
			<li>
			<div class="responsive-accordion-head active">
			<span class="bold">'.$lang['memory'].'</span>
			<i class="fa fa-chevron-down responsive-accordion-plus fa-fw iconBe-plus" style="display: none;"></i>
			<i class="fa fa-chevron-up responsive-accordion-minus fa-fw iconBe-minus" style="display: block;"></i>
			</div>
			<div class="responsive-accordion-panel active">
	...
	if (!empty($settings['show']['network'])) {
		...
		<div class="infoTable">
			...
			<li>
			<div class="responsive-accordion-head active">
			<span class="bold">'.$lang['network_devices'].'</span>
			<i class="fa fa-chevron-down responsive-accordion-plus fa-fw iconBe-plus" style="display: none;"></i>
			<i class="fa fa-chevron-up responsive-accordion-minus fa-fw iconBe-minus" style="display: block;"></i>
			</div>
			<div class="responsive-accordion-panel active">
...
echo '
		<div class="infoTable">
			...
			<table>
			    <tr>
				<td>'.getFileInfo('apache').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('mysql').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('php').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('ffmpeg').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('qt').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('yamdi').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('lame').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('unoconv').'</td>
			    </tr>
			    <tr>
				<td>'.getFileInfo('convert').'</td>
			    </tr>
			</table>
			</div>
			</li>
...
';
	...
	if (!empty($settings['show']['devices'])) {
		...
		<div class="infoTable">
			...
			<li>
			<div class="responsive-accordion-head active">
			<span class="bold">'.$lang['hardware'].'</span>
			<i class="fa fa-chevron-down responsive-accordion-plus fa-fw iconBe-plus" style="display: none;"></i>
			<i class="fa fa-chevron-up responsive-accordion-minus fa-fw iconBe-minus" style="display: block;"></i>
			</div>
			<div class="responsive-accordion-panel active">
...
<div class="infoTable">
	...
	<li>
	<div class="responsive-accordion-head active">
	<span class="bold">'.$lang['filesystem_mounts'].'</span>
	<i class="fa fa-chevron-down responsive-accordion-plus fa-fw iconBe-plus" style="display: none;"></i>
	<i class="fa fa-chevron-up responsive-accordion-minus fa-fw iconBe-minus" style="display: block;"></i>
	</div>
	<div class="responsive-accordion-panel active">
