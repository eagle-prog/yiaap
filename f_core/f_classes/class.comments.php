...
class VComments extends VView {
	...
	function commLayout($type, $vuid, $msg_arr = '', $like_arr = '') {
		...
		if ($islogged) {
			...
			$ht_js .= '$(".comm-input-action").click(function(){if($(".comm-input-action").val() == "' . $language["view.files.respond"] . $language["frontend.global." . $type[0]] . '"){$(this).val("");}$(".comm-switch").removeClass("comm-hide");$(this).removeClass("h50");});';
			/* click on cancel posting comment */
			$ht_js .= '$(".comm-cancel-action").click(function(){$(".comm-input-action").val("' . $language["view.files.respond"] . $language["frontend.global." . $type[0]] . '");$(".comm-switch2").addClass("comm-hide");$(".comm-input-action").addClass("h50");$("#comm-char-remaining").html("' . $cfg["file_comment_max_length"] . '");});';
			...
			$ht_js .= '$(".post-comment-button").click(function(){if($(".comm-input-action").val() != ""){var t = $(this); t.find("i").addClass("spinner icon-spinner"); $.post(comm_url+"&do=comm-post", $("#comm-post-form").serialize(), function(data){$("#comment-load").html(data);$(".comm-input-action").val(""); t.find("i").removeClass("spinner icon-spinner");});}});';
			...
			if ($cfg["comment_emoji"] == 1) {
				...
				//$ht_js .= '$(".cr-"+f1+" .emojionearea-editor").on("mousedown mouseup keydown keyup", getCurrentCursorPosition);';
				//$ht_js .= '$(".cr-"+f1+" .emojionearea-button").on("click", setCurrentCursorPosition);';
		...
		if ($cfg["comment_emoji"] == 1) {
			...
			$ht_js .= 'var tc = $(".comment_h p:not(.posted_on)").length; $(".comment_h p:not(.posted_on)").each(function(index,value){if(index===tc-1){$(\'#list-' . $type . '-comments-load\').hide()}var t=$(this);var c=t.text();var nc=emojione.toImage(c);t.html(nc)});';
		}
		/* linkify comments */
		...
		/* comm post response */
		$file_resp	= $islogged ? $class_database->singleFieldValue('db_' . $type . 'files', 'responding', 'file_key', $f_key) : null;
		$comm_btn	= ($cfg["file_comments"] == 1 ? '<span class="place-right showSingle-lb-off showComments" target="comments" rel="tooltip" onclick="$(\'#list-'.$type.'-comments, .comment_left_char\').toggle();$(\'#comm-post-form\').stop().slideToggle(\'fast\')" title="'.$language["subnav.entry.comments"].'"><i class="icon-eye-blocked"></i></span>' : null);
		$comm_all	= $section == $href["watch"] ? '<span class="right-float right-padding10 lh20"><span id="total-comm-nr">0</span></span> ' : null;

		$html		= '
				<div class="page_holder comm-wrapper" id="' . $type . '-comm-wrapper">
					<div class="file-views-nr"><p class="views_counter">' . $comm_all . $language["view.files.comm.all"] . $comm_btn . '</p></div><div class="clearfix"></div>
					...
					<form id="comm-post-form" method="post" action="" class="entry-form-class">
						...
						<div class="comments_actions main_comments_actions" style="display:none">
							...
							' . (($cfg["file_responses"] == 1 and $file_resp != 'none') ? '
							<a href="' . $cfg["main_url"] . '/' . VHref::getKey("respond") . '?' . $type[0] . '=' . $f_key . '" class="comm-cancel-action post-response-button place-left" rel="nofollow">' . str_replace('##TYPE##', $language["frontend.global." . $type[0]], $language["view.files.resp.post"]) . '</a>
							' : null) . '
						</div>
						</div>
					...
					' . VGenerate::simpleDivWrap('', 'list-' . $type . '-comments', self::listFileComments($type, $vuid)) . '
					' . ($section == $href["see_comments"] ? VGenerate::advHTML(22) : null) . '
		';
	...
	function listFileComments($type, $vuid, $getReplies = false) {
		...
		$total_sql	= sprintf("SELECT COUNT(*) AS `total` FROM `db_%scomments` WHERE %s AND `file_key`='%s' %s AND `c_active`='1'", $type, $reply_t, $file_key, ($edit == 1 ? "" : "AND `c_approved`='1'"));
		$comm_sql	= sprintf("SELECT 
				    ...
				    C.`comment_spam`, C.`comment_votes` 
				    FROM `db_%scomments` A, `db_accountuser` B, `db_%sfiles` C 
				    WHERE 
				    %s 
				    AND A.`file_key`='%s' 
				    AND A.`file_key`=C.`file_key` 
				    %s 
				    AND A.`c_active`='1' 
				    AND A.`c_usr_id`=B.`usr_id` 
				    ORDER BY `c_id` 
				    DESC 
				    LIMIT %s, %s;", $type, $type, $reply_s, $file_key, ($edit == 1 ? "" : "AND A.`c_approved`='1' "), $s_from, $cpp);
		...
		if ($total_nr > 0) {
						...
						<div class="comment_h" id="' . $type . '-comment' . $c_key . '">
							...
							<div class="likes_holder">
								<div class="comment-wrap comment-bg' . ($val["c_usr_id"] == intval($_SESSION["USER_ID"]) ? ' my-comm' : NULL) . '">
									...
									' . ($c_replyto == '0' ? '<div class="clearfix"></div><div class="comm-replies-show"><button id="cs-' . $c_key . '" class="search-button comm-toggle-replies" type="button" rel="tooltip" title="'.$language["view.files.replies.toggle"].'"><i class="iconBe-chevron-down"></i><span>' . $language["view.files.replies"] . '</span></button><button id="xx-' . $c_key . '" class="search-button comm-toggle-replies-quick no-display" type="button" rel="tooltip" title="'.$language["view.files.replies.toggle"].'"><i class="iconBe-chevron-down"></i><span>' . $language["view.files.replies"] . '</span></button></div>' : null) . '
								</div>
							</div>
						</div>
						<div class="clearfix"></div>
						' . self::listFileComments($type, $vuid, array("replyto" => $c_key)) . '
					</div>
			...
			if ($pages > 1 and ! $getReplies and (int) $_SESSION["USER_ID"] > 0) {
				...
				$pag = '<center><span class="' . ($page_nr != 1 ? 'comm-page-prev pointer' : 'comm-page-start') . ' left-float">' . ($page_nr != 1 ? '<a class="comm-cancel-action prev-user-comm1 paginate paginate-prev" href="javascript:;">' . $language["frontend.global.previous"] . '</a>' : '<span class="inactive">' . $language["frontend.global.previous"] . '</span>') . '</span>';

				$pag .= '<span class="' . ($page_nr != $pages ? 'comm-page-next pointer' : 'comm-page-end') . ' left-float">' . ($page_nr != $pages ? '<a class="comm-cancel-action next-user-comm1 paginate paginate-next" href="javascript:;">' . $language["frontend.global.next"] . '</a>' : '<span class="inactive">' . $language["frontend.global.next"] . '<span>') . '</span></center>';
			}
	...
	function postComment() {
		...
		/* comment permissions */
		$vcomm			= $class_database->singleFieldValue('db_' . $type . 'files', 'comments', 'file_key', $file_key);
		...
		switch ($vcomm) {
			case "all": $c_approved = 1;
				break;
			case "approve": $c_approved = 0;
				break;
			case "fronly": $c_approved = $am_fr == 1 ? 1 : 0;
				break;
		}
		/* get block status */
		...
		if ($error_message == '') {
			...
			$db_nr		= sprintf("UPDATE `db_%sfiles` SET `file_comments`=`file_comments`+1 WHERE `file_key`='%s' LIMIT 1;", $type, $file_key);
			$db_update	= $db->execute($db_nr);
			/* log comments */
			$log		= ($cfg["activity_logging"] == 1 and $action = new VActivity($_SESSION["USER_ID"])) ? $action->addTo('log_filecomment', $type . ':' . $file_key . ':' . $c_key) : NULL;
			/* mailing */
			if ($class_database->singleFieldValue('db_accountuser', 'usr_mail_filecomment', 'usr_id', $upage_id) == 1) {
				$mail_do = VNotify::queInit('file_comment', array(VUserinfo::getUserEmail($upage_id)), $db_id);
			}
			if ($_GET["do"] == 'comm-reply') {
                                $mail_do = VNotify::queInit('file_comment_reply', array(VUserinfo::getUserEmail($class_database->singleFieldValue('db_' . $type . 'comments', 'c_usr_id', 'c_key', $comm_arr["c_replyto"]))), $db_id);
                                ...
                                if ($cu->fields["c_usr_id"] > 0) {
                                        ...
                                        if ($ci->fields["c_usr_id"] > 0) {
                                                $mail_do = VNotify::queInit('file_comment_reply', array(VUserinfo::getUserEmail($ci->fields["c_usr_id"])), $db_id);
                                        }
                                }
                        }

		}
		return VComments::commLayout($type, $upage_id, array($notice_message, $error_message));
	}
	...
	function commentActions($act) {
		...
		if ($db->Affected_Rows() > 0) {
			if ($act == 'comm-delete') {
				...
				$nr_sql = sprintf("UPDATE `db_%sfiles` SET `file_comments`=`file_comments`-1 WHERE `file_key`='%s' LIMIT 1;", $type, $kk);
				$db->execute($nr_sql);
			}
	...
	function browseComment() {
		...
		return VComments::commLayout($type, $upage_id);
	}
