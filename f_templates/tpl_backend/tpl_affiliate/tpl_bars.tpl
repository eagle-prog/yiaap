...
.content-filters a, .filter-text{ldelim}color:#888;{rdelim}
...
.filter-text{ldelim}font-size:14px;font-weight:normal;margin-left:7px;padding:10px 0;display:inline-block{rdelim}
{literal}
...
$(document).ready(function(){ldelim}
	$(".view-mode-type").click(function(){ldelim}
                t = $(this);
                type = t.attr("rel-t");
                u = '{$backend_url}/{href_entry key="be_affiliate"}?o={$smarty.get.o|sanitize}&t='+type;
...
function renderWeekOverWeekChart() {
    ...
    var thisWeek = query({
      ...
      'start-date': '{$twsd}',
      'end-date': '{$twed}',
      'filters': '{if $user_filter|count_characters > 2}ga:dimension1=={$user_filter|sanitize};{/if}ga:dimension2=={$file_type}{if $smarty.get.fk ne ""};ga:dimension3=={$smarty.get.fk|sanitize}{/if}{$views_min}{$views_max}'{literal}
    });
