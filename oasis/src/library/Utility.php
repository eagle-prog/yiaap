...
function getDomFromString($to_parse)
{
    ...
    if (stristr($to_parse, "<html") === false) {
 ...
 * with html entities. This performs a save avoiding the replacement.
...
{
    $out = $dom->saveHTML();
    $out = preg_replace("/\&([a-zA-z])+\;/u", '-a-m-p-${1};', $out);
    $out = html_entity_decode($out, ENT_QUOTES | ENT_HTML5, 'UTF-8');
    $out = preg_replace("/\-a\-m\-p\-([a-zA-z])+\;/u", '&${1};', $out);
    return $out;
}
/**
